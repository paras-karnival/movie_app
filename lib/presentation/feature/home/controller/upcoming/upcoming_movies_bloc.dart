    ...
    on<GetUpcomingMoviesApiRequestEvent>((event, emit) async {
      try {
        emit(state.copyWith(
            ...
            hasError: '',
            movieResModel: null));
        MovieResModel upcomingMovieResModel = await di<AppRepo>()
            .getUpcomingMovies(language: event.language, pageNo: event.pageNo);
        emit(state.copyWith(
            showLoader: false, movieResModel: upcomingMovieResModel));
      } catch (e) {
