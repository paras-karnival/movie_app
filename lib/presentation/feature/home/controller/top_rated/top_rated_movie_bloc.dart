...
class TopRatedMovieBloc extends Bloc<TopRatedEvent, TopRatedState> {
  TopRatedMovieBloc() : super(TopRatedState(
    hasError: '',
    hasMessage: '',
    showLoader: true,
  )) {
    on<GetTopRatedMoviesApiRequestEvent>((event, emit) async{
      try {
        emit(state.copyWith(showLoader: true ,hasMessage: '',hasError: '',topRatedMovieResModel: null ));
        TopRatedMovieResModel topRatedMovieResModel =
            await di<AppRepo>().getTopRatedMovies(
            language: event.language,
            pageNo: event.pageNo
        );
        emit(state.copyWith(showLoader: false ,topRatedMovieResModel: topRatedMovieResModel));
      } catch (e) {
